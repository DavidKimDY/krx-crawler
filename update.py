...
def insert_document_test(collection, data):
    map_ = {
         '종목명': 'name',
         '시장구분': 'market',
         '소속부': 'division',
    ...
    for key in data_dict:
        eng_key = map_[key]
        value = data_dict[key]
        if key == '종목코드' and type(value['0']) != str:
            new_value = {}
            for v in value:
                wrong = str(int(value[v]))
                edited = '0' * (6 - len(wrong)) + wrong
                new_value[v] = edited
            value = new_value
...
def main():
    ...
    krx_stock_date = mu.get_mongodb_collection('krx_stock_date')
    file_list = get_file_list()

    for file in file_list:
        print(file)
        raw_data = open_file(file)
        end_key_data = convert_key_into_eng(raw_data)
        ...
        ##
        insert_document_test(krx_stock_date, data_for_krx_storage)
        ##
