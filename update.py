...
def insert_with_date(data, collection, date):
    data_copy = data.copy()
    data_copy['date'] = date
...
def main():
    ...
    krx_stock_all  = mu.get_mongodb_collection('krx_stock_all')
    ...
    for file in file_list:
        ...
        for data in data_list:
            insert_with_date(data, krx_stock, date)
            symbol = data['symbol']
            data['date'] = date
            if is_new_data(krx_stock_all, symbol):
                insert_document(krx_stock_all, {'symbol': symbol, 'data': [data]})
            else:
